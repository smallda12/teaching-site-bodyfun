...
(function () {
  ...
  const 相簿 = (function () {
    ...
    function 開() {
      return new Promise((res) => {
        if (db) return res(db);
        const r = indexedDB.open("身體一級棒照片", 1);
  ...
  const 點名 = (function () {
    ...
    async function 匯出() {
      ...
      const 資料 = { 格式: "身體一級棒點名名單", 版本: 1, 匯出時間: new Date().toISOString(), 學生: [] };
      for (const s of 名單) {
  ...
  const 教學 = (function () {
    ...
    function 畫重點清單() {
      ...
      // 🛑 `收尾` 可能是空的（收尾頁由最後一個重點承擔，例：〈再生藝術〉的
      //    P42／P43／P50 就是重點 4 的教學頁，兩邊都放會讓頁碼重複的斷言爆掉）——
      //    只看「全過」會在四個重點做完後顯示一張空白卡。空陣列時整區不顯示。
      $("#收尾區").classList.toggle("顯示", 全過 && UNIT.收尾.length > 0);
    }
  ...
  /* ================================================================
     分區八：課文導讀（選配）
     ----------------------------------------------------------------
     資料來自 data.js 的 UNIT.課文；沒有這個欄位或沒有段落時，整個分區
     與導覽鈕都會自動隱藏（沿用「延伸」分區同一套「資料驅動、選配」寫法，
     舊單元、其他領域單元不用改也不會出現這個分區）。
     ================================================================ */
  const 導讀 = (function () {
    const 有資料 = () =>
      window.UNIT.課文 && Array.isArray(UNIT.課文.段落) && UNIT.課文.段落.length > 0;

    function 畫() {
      if (!有資料()) {
        $("#區課文")?.remove();
        $$(".導覽鈕").find((b) => b.dataset.區 === "區課文")?.remove();
        return;
      }
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    導讀.畫();        /* ★ 同上，要在 初始化分頁() 之前，沒有課文資料時才來得及移除導覽鈕 */
    初始化分頁();
    點名.初始化();
    輪播.初始化();
    教學.初始化();
